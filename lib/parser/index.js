...
function cmpTokenValue(token, referenceStr) {
    return cmpStr(scanner.source, token.start, token.end, referenceStr);
}
...
function parseError(message) {
    ...
    var location = scanner.token !== null
        ? scanner.getLocation(scanner.token.start)
        : scanner.findLastNonSpaceLocation();
...
function getInfo() {
    if (needPositions && scanner.token !== null) {
        return scanner.getLocation(scanner.token.start, filename);
    }
...
function getNamespacedIdentifier(checkColon) {
    ...
    if (scanner.token !== null) {
        if (scanner.token.type === VERTICALLINE &&
            scanner.lookupType(1) !== EQUALSSIGN) {
            len++;
...
function checkProgid() {
    ...
    for (var type; type = scanner.lookupType(offset); offset++) {
        if (type === RIGHTPARENTHESIS) {
            return scanner.currentToken + offset;
        }
    }
...
function getProgid(progidEnd) {
    ...
    var start = scanner.token.start;
    var end;
    for (; scanner.token !== null && scanner.currentToken < progidEnd; scanner.next()) {
        end = scanner.token.end;
    }
