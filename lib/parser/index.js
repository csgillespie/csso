...
function getDeclaration(nested) {
    ...
    // check it's a filter
    value = cmpStr(name, nameLength - 6, nameLength, 'filter') && checkProgid()
        ? getFilterValue()
        : getValue(nested);
...
function tryGetPercentage() {
    ...
    if (number && scanner.tokenType === PERCENTSIGN) {
        return getPercentage(number);
    }
...
function scanUnicodeRange(tryNext) {
    ...
    for (; !scanner.eof; scanner.next()) {
        ...
        hexLength += scanner.tokenEnd - scanner.tokenStart;
    }
    ...
    for (var i = 0; i < hexLength; i++) {
        if (isHex(scanner.source.charCodeAt(hexStart + i)) === false) {
            scanner.error('Unexpected input');
        }
    }
    ...
    if (tryNext) {
        if (scanner.tokenType === HYPHENMINUS) {
            ...
            var next = scanUnicodeRange(false);
...
// odd | even | number? n
function getNth() {
    scanner.expectAny('Number, odd or even',
        IDENTIFIER,
        DECIMALNUMBER
    );
    ...
    if (scanner.tokenType === DECIMALNUMBER) {
        if (scanner.lookupType(1) === IDENTIFIER && scanner.lookupValue(1, 'n')) {
            scanner.next();
        }
    } else {
        if (scanner.lookupValue(0, 'n') === false &&
            scanner.lookupValue(0, 'odd') === false &&
            scanner.lookupValue(0, 'even') === false) {
            scanner.error('Unexpected identifier');
        }
    }
...
function getNthSelector() {
    ...
    scanner.eat(COLON);
    scanner.expectIdentifier('nth', false);
...
function getProgid(progidEnd) {
    ...
    if (!progidEnd) {
        scanner.error('progid is expected');
    }
...
function getPseudo() {
    ...
    if (nextType === NULL) {
        scanner.next();
        scanner.error('Colon or identifier is expected');
    }
    ...
    if (nextType === IDENTIFIER && scanner.lookupValue(1, 'nth')) {
