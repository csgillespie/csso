...
function getStylesheet(nested) {
    ...
    while (scanner.token !== null) {
        ...
        if (child !== null) {
            node.rules.append(List.createItem(child));
        }
    }
...
function getAtruleExpression() {
    ...
    while (scanner.token !== null) {
        ...
        if (child !== null) {
            node.sequence.append(List.createItem(child));
        }
    }
    ...
    while (scanner.token !== null) {
        switch (scanner.token.type) {
            ...
            default:
                ...
                lastComma = false;
                node.selectors.append(List.createItem(getSimpleSelector()));
...
function getSimpleSelector(nested) {
    ...
    while (scanner.token !== null) {
        ...
        if (child !== null) {
            if (combinator !== null) {
                node.sequence.append(List.createItem(combinator));
                combinator = null;
            }
...
function getDeclarations() {
    ...
    while (scanner.token !== null) {
        ...
        if (child !== null) {
            declarations.append(List.createItem(child));
        }
    }
...
function getBraces(scope) {
    ...
    var node = {
        type: 'Braces',
        info: getInfo(),
        open: getTokenValue(scanner.token),
        close: null,
        sequence: new List()
    };
    ...
    while (scanner.token !== null) {
        switch (scanner.token.type) {
            case close:
                node.close = getTokenValue(scanner.token);
                break scan;
...
function getFunctionArguments(scope) {
    ...
    while (scanner.token !== null) {
        ...
        if (argument === null) {
            ...
            args.append(List.createItem(argument));
        }

        if (child !== null) {
            argument.sequence.append(List.createItem(child));
        }
    }
...
function getNotFunctionArguments() {
    ...
    while (scanner.token !== null) {
        switch (scanner.token.type) {
            ...
            default:
                wasSelector = true;
                args.append(List.createItem(getSimpleSelector(true)));
        }
    }
...
function getVarFunctionArguments() { // TODO: special type Variable?
    ...
    args.append(List.createItem({
        type: 'Argument',
        sequence: new List().fromArray([getIdentifier(true)])
    }));
    ...
    if (scanner.token !== null && scanner.token.type === COMMA) {
        ...
        args.append(List.createItem({
            type: 'Argument',
            sequence: new List().fromArray([getValue(true)])
        }));
...
function getOldIEExpression(scope, ident) {
    ...
    return {
        ...
        info: ident.info,
        name: ident.name,
        arguments: new List().fromArray([{
            type: 'Argument',
            sequence: new List().fromArray([{
                type: 'Raw',
                value: scanner.source.substring(start, end)
            }])
        }])
...
function getNthSelector() {
    ...
    while (scanner.token !== null) {
        ...
        if (child !== null) {
            sequence.append(List.createItem(child));
        }
    }
...
function getFilterValue() { // TODO
    ...
    while (progid = checkProgid()) {
        node.sequence.append(List.createItem(getProgid(progid)));
    }
...
function getString() {
    var node = {
        type: 'String',
        info: getInfo(),
        value: getTokenValue(scanner.token).replace(/\\(\n|\r\n|\r|\f)/g, '') // TODO: remove regexp
    };
