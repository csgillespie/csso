            ...
            if (!diff.ne1.length && !diff.ne2.length) {
                ...
                if (diff.ne1.length && !diff.ne2.length) {
                    ...
                    if (selectorLength < blockLength) {
                        ...
                        block.declarations = new List().fromArray(diff.ne1);
                    }
                    ...
                    if (selectorLength < blockLength) {
                        utils.addSelectors(selectors, prevSelectors);
                        prevBlock.declarations = new List().fromArray(diff.ne2);
                    }
                } else {
                    ...
                    if (blockLength >= newBlockLength) {
                        var newRuleset = {
                            ...
                            block: {
                                type: 'Block',
                                info: {},
                                declarations: new List().fromArray(diff.eq)
                            }
                        };

                        block.declarations = new List().fromArray(diff.ne1);
                        prevBlock.declarations = new List().fromArray(diff.ne2.concat(diff.ne2overrided));
